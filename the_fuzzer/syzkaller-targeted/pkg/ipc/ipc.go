...
type CallInfo struct {
	...
	CorrCover []uint32 // corruption coverage
...
func (env *Env) Exec(opts *ExecOpts, p *prog.Prog) (output []byte, info *ProgInfo, hanged bool, err0 error) {
	return env.ExecWithSleep(opts, p, uint64(9999999), 0, nil)
}

func (env *Env) ExecWithTracking(opts *ExecOpts, p *prog.Prog) (output []byte, info *ProgInfo, hanged bool, err0 error) {
	return env.ExecWithSleep(opts, p, uint64(9999999), 1, nil)
}

func (env *Env) ExecWithSleep(opts *ExecOpts, p *prog.Prog, sleepIdx uint64, trackObjects uint8, pauseChannel chan struct{}) (output []byte, info *ProgInfo, hanged bool, err0 error) {
	...
	output, hanged, err0 = env.cmd.execWithSleep(opts, progData, sleepIdx, trackObjects, pauseChannel, len(p.Calls))
	if err0 != nil {
		env.cmd.close()
		env.cmd = nil
		return
...
func (c *command) handshakeError(err error) error {
	c.cmd.Process.Kill()
	output := <-c.readDone
	err = fmt.Errorf("IPC-executor %v: %v\n%s", c.pid, err, output)
	c.wait()
	return err
}
...
func (c *command) exec(opts *ExecOpts, progData []byte) (output []byte, hanged bool, err0 error) {
	return c.execWithSleep(opts, progData, uint64(9999999), 0, nil, 0)
}

func (c *command) execWithSleep(opts *ExecOpts, progData []byte, sleepIdx uint64, trackObjects uint8, pauseChannel chan struct{}, progLen int) (output []byte, hanged bool, err0 error) {
	scallTimeout := uint64(c.config.Timeouts.Syscall / time.Millisecond)
	progTimeout := uint64(c.config.Timeouts.Program / time.Millisecond)
	sdownScale := uint64(c.config.Timeouts.Scale)
	if sleepIdx != 9999999 {
		scallTimeout += 1000 // 2000 ms for per-syscall timeout
		// sdownScale = 1                                // 1000 ms slowdown
		progTimeout += scallTimeout * uint64(progLen) // max timeout of `N` syscalls
	}
	if trackObjects > 0 {
		progTimeout += 2000 // 2000 ms for per-program timeout
	}

	fmt.Printf("[.] IPC-executor: executing prog with parameters: sleepIdx=%d trackObjects=%d pauseChannel=%v\n",
		sleepIdx, trackObjects, pauseChannel)
	...
	req := &executeReq{
		...
		pid:              uint64(c.pid),
		syscallTimeoutMS: scallTimeout,
		programTimeoutMS: progTimeout,
		slowdownScale:    sdownScale,
		progSize:         uint64(len(progData)),

		sleepAtIndex: sleepIdx, // this is to make sure that our executor knows to sleep at certain program location
		trackObjects: trackObjects,
	}
	reqData := (*[unsafe.Sizeof(*req)]byte)(unsafe.Pointer(req))[:]
	if _, err := c.outwp.Write(reqData); err != nil {
		output = <-c.readDone
		err0 = fmt.Errorf("[!] IPC-executor %v: failed to write control pipe: %v", c.pid, err)
		return
	}
	if progData != nil {
	...
	for {
		...
		if reply.magic == 69420 && sleepIdx != 9999999 && reply.status == outMagic {
			fmt.Printf("[.] IPC-executor: Got the reply! Let's keep executing the corruption... sleepIdx=%d magic=%d\n", sleepIdx, reply.magic)
			pauseChannel <- struct{}{}
			continue
		}
		if reply.magic != outMagic && reply.magic != 69420 {
			fmt.Fprintf(os.Stderr, "[!] IPC-executor %v: got bad reply magic 0x%x\n", c.pid, reply.magic)
			os.Exit(1)
		}
		if reply.done != 0 {
		...
		if callReply.signalSize != 0 || callReply.coverSize != 0 || callReply.compsSize != 0 {
			...
			fmt.Fprintf(os.Stderr, "[!] IPC-executor %v: got call reply with coverage\n", c.pid)
			os.Exit(1)
		}
	...
	if exitStatus == statusFail {
		err0 = fmt.Errorf("[!] IPC-executor %v: exit status %d\n%s", c.pid, exitStatus, output)
	}
	return
}
