...
type Config struct {
	...
	// This is an important mapping to restrict the fuzzer scope for functions of interest only (default: "")
	StructToFunctions string `json:"struct_funct_mapping"`

	StructOffsetSizeFile string `json:"struct_offsets_sizes"`

	// This option is in the cases when we want to track all object allocations (default: false)
	TotalTrackingMode bool `json:"total_tracking_mode"`

	// EnableSelectiveCoverage bool `json:"enable_selective_coverage"`
	// Addr2LinePath string `json:"addr2line_path"`
}
