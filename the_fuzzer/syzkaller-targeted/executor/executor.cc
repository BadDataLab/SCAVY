...
// tunable sleep values to wait the execution
...
#if SYZ_EXECUTOR_USES_FORK_SERVER
	fprintf(stderr, "[+] executor.cc: fork server exited with status %d\n", status);
...
void receive_execute()
{
	...
	flag_threaded = 0; //req.exec_flags & (1 << 3); // Disables collide and threaded
	flag_collide = 0; //req.exec_flags & (1 << 4);
	flag_coverage_filter = req.exec_flags & (1 << 5);
	if (req.sleep_at_index == 9999999) {
		sleep_at_idx = -1;
	} else {
		sleep_at_idx = req.sleep_at_index;
	}
	if (req.track_objects == 1 || sleep_at_idx != -1) {
		track_objects = true;
		debug("[+] tracking objects...\n");
	}
	debug("[+] syz-executor: Got track_objects=%d and sleep_at_index=%d slowdown_scale=%d!\n",req.track_objects,sleep_at_idx,slowdown_scale);
	if (!flag_threaded)
		flag_collide = false;
...
void reply_waiting_corruption()
{
	execute_reply reply = {};
	reply.magic = (uint32)69420;
	reply.done = false;
	reply.status = kOutMagic;
	if (write(kOutPipeFd, &reply, sizeof(reply)) != sizeof(reply))
		fail("control pipe write failed");
}
...
retry:
	...
	for (;;) {
		if (instruction_idx == sleep_at_idx) {
			debug("[.] syz-executor: Sleeping at index %d...\n",instruction_idx);
			kprobe::stop_tracing();
			reply_waiting_corruption();
			sleep_ms(2000);
			kprobe::clear_traces();
		}
		// this was to slow down every syscall
		// if (sleep_at_idx != -1) { 
		// 	sleep_ms(1500);
		// }
		instruction_idx++;
		...
		if (prog_extra_timeout < call->attrs.prog_timeout)
			prog_extra_timeout = call->attrs.prog_timeout * slowdown_scale; // this is where timeout calculation happens
		...
		// } else if (flag_threaded) {    /////////////////////////// We make sure this code doesnt run
