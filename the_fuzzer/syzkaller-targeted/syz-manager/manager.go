...
type Manager struct {
	...
	StructToFunct        map[string][]string
	StructOffsetSize     map[string]map[int]int
	TotalCorrputionStats alloc_corrupter.TotalCorruptionStats
}
...
func RunManager(cfg *mgrconfig.Config) {
	...
	mgr.loadStructToFuncMapping()     // loads the mapping between structure and function that uses it.
	mgr.loadStructOffsetSizeMapping() // loads the offset-size mapping for each structure
	if mgr.cfg.TotalTrackingMode && (mgr.StructToFunct != nil) {
		log.Fatalf("[ERROR] Cannot have both targeted fuzzing and total tracking!")
	}

	mgr.loadCorruptionStats()
	...
	go func() {
		for lastTime := time.Now(); ; {
			...
			// 	numFuzzing, executed, corpusCover, corpusSignal, maxSignal, crashes, numReproducing)  // Disabled this logging for now
		}
	}()

	if *flagBench != "" {
		f, err := os.OpenFile(*flagBench, os.O_APPEND|os.O_WRONLY|os.O_CREATE, osutil.DefaultFilePerm)
		if err != nil {
		...
		go func() {
			for {
				...
				vals["firstconnect"] = uint64(mgr.firstConnect.Unix())
				mgr.mu.Unlock()
...
func (mgr *Manager) loadStructToFuncMapping() {
	file, err := os.Open(mgr.cfg.StructToFunctions)
	if err == nil {
		dataDecoder := json.NewDecoder(file)
		dataDecoder.Decode(&mgr.StructToFunct)
		file.Close()
	} else {
		mgr.StructToFunct = nil
	}
}

func (mgr *Manager) loadStructOffsetSizeMapping() {
	if len(mgr.cfg.StructOffsetSizeFile) <= 0 {
		mgr.StructOffsetSize = make(map[string]map[int]int)
		return
	}
	file, err := os.Open(mgr.cfg.StructOffsetSizeFile)
	if err == nil {
		dataDecoder := json.NewDecoder(file)
		dataDecoder.Decode(&mgr.StructOffsetSize)
		log.Logf(3, "[+] MANAGER: Loaded the structures' offsets and sizes --> %v", mgr.StructOffsetSize)
		file.Close()
	} else {
		log.Logf(3, "[!] MANAGER: Couldn't open struct offset file at %v", mgr.cfg.StructOffsetSizeFile)
		mgr.StructOffsetSize = make(map[string]map[int]int)
	}
}

func (mgr *Manager) loadCorruptionStats() {

	fi, err := os.Open("CorruptionData.gob")
	if err != nil {
		mgr.TotalCorrputionStats = alloc_corrupter.TotalCorruptionStats{
			TotalNumHits:     0,
			CorruptionsStats: make([]alloc_corrupter.CorruptionStats, 0, 20),
		}
		log.Logf(0, "[.] Corruption statistics file not found! Starting fresh!\n")
		mgr.saveCorruptionStats()
		return
	}
	defer fi.Close()
	...
	if err != nil {
		log.Fatalf("[!] In Manager.loadCorruptionStats() gzip reader: %v\n", err)
	}
	defer fz.Close()

	decoder := gob.NewDecoder(fz)
	err = decoder.Decode(&mgr.TotalCorrputionStats)
	if err != nil {
		log.Fatalf("[!] In Manager.loadCorruptionStats() decode: %v\n", err)
	}
}

func (mgr *Manager) saveCorruptionStats() {
	fi, err := os.Create("CorruptionData.gob")
	if err != nil {
		log.Logf(0, "[!] In Manager.saveCorruptionStats: Corruption statistics file not saved (error in file open): %v!\n", err)
	}
	defer fi.Close()
	...
	err = encoder.Encode(&mgr.TotalCorrputionStats)
	if err != nil {
		log.Logf(0, "[!] In Manager.saveCorruptionStats: Corruption statistics file not saved (error in encoding): %v!\n", err)
	}
}
...
func (mgr *Manager) runInstanceInner(index int, instanceName string) (*report.Report, []byte, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to copy fuzzer binary: %v", err)
	}

	// if mgr.cfg.EnableSelectiveCoverage {
	// 	_, err = inst.Copy(filepath.Join(mgr.cfg.KernelObj, mgr.sysTarget.KernelObject))
	// 	if err != nil {
	// 		return nil, nil, fmt.Errorf("failed to copy vmlinux: %v", err)
	// 	}
	// 	if mgr.cfg.Addr2LinePath != "" {
	// 		_, err = inst.Copy(filepath.Join(mgr.cfg.Addr2LinePath))
	// 		if err != nil {
	// 			return nil, nil, fmt.Errorf("failed to copy addr2line binary: %v", err)
	// 		}
	// 	}
	// }
	...
	if executorBin == "" {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("failed to copy executor binary: %v", err)
		}
	}
...
func (mgr *Manager) NewCorruptionInput(inp *rpctype.RPCCorruptionInput) int {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	toreturn, _ := mgr.TotalCorrputionStats.InsertCorruptionStats(&inp.CorrStats)
	mgr.saveCorruptionStats()
	log.Logf(3, "[+] syz-manager: Saved new corruption stats at index %d!\n", toreturn)
	log.Logf(3, "[.] Got corruption: %v\n", *inp.CorrStats.Corruption)
	return toreturn
}
