...
type RPCManagerView interface {
	...
	NewCorruptionInput(inp *rpctype.RPCCorruptionInput) int
...
func startRPCServer(mgr *Manager) (*RPCServer, error) {
	serv := &RPCServer{
		...
		StructToFunct:        &mgr.StructToFunct,
		StructOffsetSize:     &mgr.StructOffsetSize,
		TotalCorrputionStats: &mgr.TotalCorrputionStats,
	}
	serv.batchSize = 5
...
func (serv *RPCServer) Connect(a *rpctype.ConnectArgs, r *rpctype.ConnectRes) error {
	...
	r.TotalTracking = serv.cfg.TotalTrackingMode
	r.StructToFunct = *serv.StructToFunct
	r.StructOffsetSize = *serv.StructOffsetSize
	r.SavedCorruptionStats = *serv.TotalCorrputionStats
...
func (serv *RPCServer) NewCorruptionInput(a *rpctype.RPCCorruptionInput, r *rpctype.PollRes) error {
	serv.mu.Lock()
	defer serv.mu.Unlock()
	toRet := serv.mgr.NewCorruptionInput(a)
	if toRet >= 0 {
		return nil
	} else {
		return errors.New("Some problem inserting new corruption stats!\n")
	}
}
...
func (serv *RPCServer) SaveAllocations(alloc_map map[uint64]map[string]int, r *rpctype.PollRes) error {
	serv.mu.Lock()
	defer serv.mu.Unlock()

	var allocated_addr_mapping = make(map[uint64]map[string]int)

	// restore older allocations
	file, err := os.Open("allocated_mapping.json")
	if err == nil {
		dataDecoder := json.NewDecoder(file)
		dataDecoder.Decode(&allocated_addr_mapping)
		file.Close()
	}
	log.Logf(0, "[.] Read map of size %d from file", len(allocated_addr_mapping))

	// merge allocations
	for k, v := range alloc_map {
		if _, ok := allocated_addr_mapping[k]; ok {
			for k1 := range v {
				allocated_addr_mapping[k][k1] = 1
			}
		} else {
			allocated_addr_mapping[k] = v
		}
	}
	log.Logf(0, "[.] Merged allocation with new input of size %d", len(alloc_map))

	// save latest allocations
	file, err = os.Create("allocated_mapping.json")
	if err != nil {
		panic(err)
	}

	dataEncoder := json.NewEncoder(file)
	dataEncoder.Encode(allocated_addr_mapping)
	file.Close()
	log.Logf(0, "[.] Saved map of size %d from file", len(allocated_addr_mapping))
	return nil
}
