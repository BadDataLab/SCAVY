...
func (wq *WorkQueue) enqueue(item interface{}) {
	...
	case *WorkCorrupt:
		wq.corrupt = append(wq.corrupt, item)
	default:
...
func (wq *WorkQueue) dequeue() (item interface{}) {
	...
	// corruption workqueue takes precedence from any other
	if len(wq.corrupt) != 0 {
		last := len(wq.corrupt) - 1
		item = wq.corrupt[last]
		wq.corrupt = wq.corrupt[:last]
