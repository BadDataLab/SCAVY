...
type Fuzzer struct {
	...
	targetedFuzzing  map[string][]string    // if this is nil we have the good ol' Syzkaller
	structOffsetSize map[string]map[int]int // this is a mapping used to find what to corrupt and where
	totalTracking    bool
...
func main() {
	...
	fuzzer := &Fuzzer{
		...
		totalTracking:            r.TotalTracking,
		targetedFuzzing:          r.StructToFunct,
		structOffsetSize:         r.StructOffsetSize,

		totalCorrputionStats: r.SavedCorruptionStats,
	}
	...
	// initialize the corrupter
	if fuzzer.targetedFuzzing != nil {
		if inited, err := alloc_corrupter.GlobalCorrupterState.InitializeWithStats(&fuzzer.totalCorrputionStats); !inited || err != nil {
			log.Fatalf("[!] Problem with initializing the connection with the corrupter module!")
		}
	}

	alloc_corrupter.Total_alloc_tracking_mode = fuzzer.totalTracking
	alloc_corrupter.InitializeKprobe()
	go alloc_corrupter.ThreadedKprobeReader()
...
func (fuzzer *Fuzzer) pollLoop() {
	...
	for {
		...
		if fuzzer.totalTracking {
			fuzzer.pushAllocations() // push the allocations we have captured so far
		}
	}
}
...
func (fuzzer *Fuzzer) poll(needCandidates bool, stats map[string]uint64) bool {
	...
	// log.Logf(1, "poll: candidates=%v inputs=%v signal=%v", // Disabled this logging for now
...
func (fuzzer *Fuzzer) saveCorruptionStats(cstats *alloc_corrupter.CorruptionStats) {
	a := &rpctype.RPCCorruptionInput{
		CorrStats: *cstats,
	}
	r := &rpctype.PollRes{}
	if err := fuzzer.manager.Call("Manager.NewCorruptionInput", a, r); err != nil {
		log.Fatalf("Manager.NewCorruptionInput call failed: %v", err)
	}
}

func (fuzzer *Fuzzer) pushAllocations() bool {
	alloc_corrupter.TotalTrackingVariablesLock()
	defer alloc_corrupter.TotalTrackingVariablesUnlock()

	r := &rpctype.PollRes{}
	if err := fuzzer.manager.Call("Manager.SaveAllocations", alloc_corrupter.KprobeGetTotalMap(), r); err != nil {
		log.Fatalf("Manager.SaveAllocations call failed: %v", err)
	}

	log.Logf(0, "[.] Fuzzer sending mapping of size %d to manager", len(alloc_corrupter.KprobeGetTotalMap()))
	return true
}
...
func (fuzzer *Fuzzer) checkNewSignal(p *prog.Prog, info *ipc.ProgInfo) (calls []int, extra bool) {
	...
	if p.CorrIndex < 1 {
		...
		// todo add corruptionHits here too
		for i, inf := range info.Calls {
			if i < p.CorrIndex {
				continue
			}
